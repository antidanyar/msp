...
def spacify(word):
        ...
        word = word.replace(" /", " |")
        word = word.replace("-","|")
        word = re.sub(r"\|+( )*\|+", "|", word) #this happens when - is replaced with |
        word = word.replace("|", '#') #for n-gram purposes; DK
        return word
...
def palatalize(word):
        ...
        word=re.sub(r"([aeiouáéíóú]) j (\| )*([ií])", "\\1 \\2 \\3", word) 
...
def transcribe(word, stress, spaces, voice, reduction):
        ...
        if spaces == "no":
                ...
                word = word.replace(" ", "-")    #DK: for my own purposes 
...
HWcons = {
	 "ʦ" : "ts",
	 "ʐ" : "zh",
	 "ɕ" : "shshj", #added j for stuff to work: DK
         "ʑ": "zhzhj",#added for stuff to work: DK
	 "ʂ" : "sh", 
         "ɣ": "gh", #added for stuff to work: DK
	 "ʨ" : "ch", 
	 "ɡ" : "g",
	 "ʲ" : "j",
	 "ə" : "ee"}
...
def UCLAPL_transcribe(word, UCLAPL="nostress"): #def added DK
